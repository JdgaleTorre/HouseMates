      ...
      {cleaning ? (
        <>
          ...
          <Text className="text-sm text-slate-700">
            Up now: <Text className="font-semibold">{nameFor(getAssigneeForPeriod(cleaning, 0), memberProfiles)}</Text>
          </Text>
          <Text className="text-sm text-slate-500">
            Up next: {nameFor(getAssigneeForPeriod(cleaning, 1), memberProfiles)}
          </Text>
        </>
      ) : null}
    </Pressable>
